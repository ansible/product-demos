...
<head>
  ...
  <style>
    ...
    .event { margin: 0.75rem 0; padding: 1rem 1.25rem; background: #e8f5e9; border-left: 6px solid #2e7d32;
             border-radius: 8px; box-shadow: 0 1px 2px rgba(0,0,0,.08); }
...
<body>
  ...
  <p class="lead">
    Edit <code>/etc/ssh/sshd_config</code> on a RHEL worker. When drift is detected,
    a friendly event appears here. Then open AAP and confirm remediation fired.
  </p>
  <div id="events" class="waiting">Waiting for sshd_config changes…</div>
  <p class="footer">Topic: linux-audit-events · filtered for sshd_config_change</p>
  <script>
    async function refresh() {
      ...
      root.innerHTML = data.map(e =>
        `<div class="event"><strong>${e.headline}</strong><span>${e.detail} · ${e.time}</span></div>`
      ).join('');
    }
