...
### Guardrail: do not modify controller notifications
- Do not edit any `controller_notifications` records in any file.
- Specifically, preserve notification objects and fields exactly as they exist today (names, URLs, methods, headers, and template bindings).
- The migration scope is limited to projects, templates, workflows, surveys, and local playbook entrypoints.
...
- Keep all existing notification associations on templates unchanged.
...
- `controller_notifications` blocks are byte-for-byte unchanged from pre-migration state.
